...
# TODO: RESET THESE EACH TIME
IMAGE_HEIGHT = 256
IMAGE_WIDTH = 256
IMAGE_DEPTH = 3
NUM_CLASSES = 6
NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN = 24 #TODO:PLAY WITH
NUM_EXAMPLES_PER_EPOCH_FOR_TEST = 24 #TODO:PLAY WITH
NUM_EXAMPLES_PER_EPOCH_FOR_EVAL = 1 #TODO:PLAY WITH
...
def get_all_test_data(im_list, la_list):
  ...
  for im_filename, la_filename in zip(im_list, la_list):
    ...
    la = la[:,:,1]
    #la = np.reshape(la, [la.shape[0], la.shape[1], 1])
    la = la[np.newaxis]
    la = la[...,np.newaxis]
    images.append(im)
    labels.append(la)
